...
export default function GuideListPage() {
  ...
  const [navigatingGuideId, setNavigatingGuideId] = useState(null);

  const handleOpenProfile = (guideId) => {
    setNavigatingGuideId(guideId);
    navigate(`/guide/${guideId}`);
  };

  const handleStartBooking = (guideId) => {
    setNavigatingGuideId(guideId);
          ...
          <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {guides.map((guide) => (
              <GuideCard
                ...
                isNavigating={navigatingGuideId === guide.id}
              />
            ))}
          </section>
        )}
